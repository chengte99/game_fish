...
export var sound_manager = {
    ...
    set_music_mute: function(b_mute) {
        ...
        if (this.b_music_mute === 1) { // 静音
            ...
            if (this.bg_music) {
                cc.audioEngine.playMusic(this.bg_music, this.bg_music_loop)
            }
    ...
    // 播放背景音乐
    play_music: function(clip, loop) {
        cc.audioEngine.stopMusic(); // 先停止当前正在播放的;
        this.bg_music = clip; // 保存我们当前正在播放的背景音乐;
